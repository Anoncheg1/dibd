 ...
 * @since sonews/0.5.0
 */
class ChannelWriter extends DaemonThread {
        ...
        while (isRunning()) {
            try {
                ...
                while (it.hasNext()) {
                    ...
                    connection = ConnectionCollector.getInstance().get(socketChannel);
                ...
                if (selKey != null) {
                    try {
                        ...
                        //Log.get().log(Level.WARNING, "ChannelWriter. Error writing to channel: {0}", ex.getMessage());
                        Log.get().info("Connection to remote host:"+socketChannel.getRemoteAddress().toString()+" closed rough.");

                        // Cancel write events for this channel
                        selKey.cancel();
                        if (connection != null) {
                            connection.shutdownInput();
                            if(!ex.getMessage().equals("Broken pipe")) //will cause ClosedChannelException
            ...
            synchronized (socketChannel) {
            	...
            	if(tls != null)
            		...
            		do{
            			...
            			// block
            			if (socketChannel.write(buf) <= 0) { //no need to worry if bytes remains we will get the same buf at next loop. 
